...
def chart_evolucion(df_atleta: pd.DataFrame, metrica: str, label: str):
    ...
    fig.update_layout(title=dict(text=f"Evolución — {label}", font=dict(color="#c8d8f0", size=13)), xaxis=dict(tickfont=dict(color="#4a6080"), gridcolor="#1f2d45"), yaxis=dict(tickfont=dict(color="#4a6080"), gridcolor="#1f2d45"), paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(16,26,48,0.6)", height=220, margin=dict(l=10, r=10, t=40, b=10))
    return fig
...
def chart_barras_grupo(df: pd.DataFrame, metrica: str, label: str):
    ...
    fig.update_layout(title=dict(text=label, font=dict(color="#c8d8f0", size=13)), xaxis=dict(tickfont=dict(color="#4a6080"), gridcolor="#1f2d45"), yaxis=dict(tickfont=dict(color="#c8d8f0"), tickfont_size=11), paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(16,26,48,0.6)", height=max(250, 35 * len(df_sorted)), margin=dict(l=10, r=50, t=40, b=10))
    return fig
...
def generar_pdf_informe(datos: dict, puntos_act: dict, puntos_prev: dict | None = None) -> BytesIO:
    ...
    def hexagono(cx, cy, r, color_fill, color_stroke, lw=2):
        pts = [(cx + r * _math.cos(_math.radians(60 * i - 30)), cy + r * _math.sin(_math.radians(60 * i - 30))) for i in range(6)]
    ...
    y_centro = H - 320
    fill(AZUL_PANEL); c.roundRect(30, y_centro - 40, W - 60, 80, 6, fill=1, stroke=0); rect_s(30, y_centro - 40, W - 60, 80, CIAN, 1)
    fill(CIAN); c.setFont("Helvetica", 8); c.drawString(48, y_centro + 30, "INFORME DE EVALUACIÓN  ·  ATLETA")
    fill(BLANCO); c.setFont("Helvetica-Bold", 22); c.drawString(48, y_centro + 8, datos["nombre"].upper())
    fill(GRIS_LABEL); c.setFont("Helvetica", 10); c.drawString(48, y_centro - 22, f"{datos['deporte'].upper()}   ·   {datos['edad']} años   ·   {datos['peso']} kg   ·   {datos['estatura']} m")

    fichas = [("DEPORTE / POSICIÓN", datos["deporte"]), ("EDAD", f"{datos['edad']} años"), ("PESO", f"{datos['peso']} kg"), ("ESTATURA",f"{datos['estatura']} m")]
    ficha_w = (W - 60) / 4
    ...
    barra_y = badge_cy - 110; barra_w_total = 320; barra_x0 = (W - barra_w_total) / 2
    fill(GRIS_LABEL); c.setFont("Helvetica-Bold", 7); c.drawCentredString(W / 2, barra_y + 20, "PERFIL DE CAPACIDADES")

    for i, (cat, val) in enumerate(puntos_act.items()):
        bx = barra_x0; by = barra_y - i * 22
        fill(GRIS_LABEL); c.setFont("Helvetica", 8); c.drawString(bx, by, cat); rect_f(bx + 110, by, barra_w_total - 110, 10, (0.08, 0.14, 0.26))
        if puntos_prev and cat in puntos_prev:
            vp = min(puntos_prev[cat], 10) / 10; fill((0.35, 0.42, 0.55)); c.rect(bx + 110, by, (barra_w_total - 110) * vp, 10, fill=1, stroke=0)
        va = min(val, 10) / 10; color_b = VERDE if val >= 6.5 else NARANJA if val >= 5 else ROJO
        rect_f(bx + 110, by, (barra_w_total - 110) * va, 10, color_b); fill(BLANCO); c.setFont("Helvetica-Bold", 8); c.drawRightString(bx + barra_w_total + 28, by + 2, f"{val:.1f}")
    ...
    fill(GRIS_LABEL); c.setFont("Helvetica", 9); c.drawString(24, H - 50, f"{datos['nombre'].upper()}  ·  {datos['deporte']}  ·  {datos['fecha']}"); fill(GRIS_LABEL); c.setFont("Helvetica", 8); c.drawRightString(W - 20, H - 38, "02")
    ...
    rect_f(6, y_t - fila_h, W - 6, fila_h, AZUL_HEADER); headers = ["PRUEBA", "RESULTADO", "RENDIMIENTO", "PUNT.", "NIVEL"]
    for hx, htxt in zip(col_x, headers): fill(CIAN); c.setFont("Helvetica-Bold", 8); c.drawString(hx + 6, y_t - fila_h + 8, htxt)
    y_t -= fila_h

    for i, (prueba, val_raw, unidad, max_v) in enumerate(metricas_tabla):
        bg = AZUL_FILA_A if i % 2 == 0 else AZUL_FILA_B; rect_f(6, y_t - fila_h, W - 6, fila_h, bg); fill(GRIS_TEXTO); c.setFont("Helvetica", 9); c.drawString(col_x[0] + 6, y_t - fila_h + 8, prueba)
        val_fmt = f"{val_raw:.2f}".rstrip("0").rstrip(".") if isinstance(val_raw, float) else str(val_raw); fill(BLANCO); c.setFont("Helvetica-Bold", 9); c.drawString(col_x[1] + 6, y_t - fila_h + 8, f"{val_fmt} {unidad}".strip())

        if max_v and float(val_raw) > 0:
            puntaje = min((float(val_raw) / max_v) * 10, 10); color_b = VERDE if puntaje >= 6.5 else NARANJA if puntaje >= 5 else ROJO
    ...
    y_seccion = y_t - 20; seccion_h = 220
    fill(CIAN); c.setFont("Helvetica-Bold", 8); c.drawString(24, y_seccion, "PERFIL RADAR DE CAPACIDADES")

    radar_cx = 130; radar_cy = y_seccion - 110; radar_r  = 85; n_pts    = len(puntos_act); cats     = list(puntos_act.keys())
    def radar_punto(idx, radio): ang = idx * (2 * _math.pi / n_pts) - _math.pi / 2; return radar_cx + radio * _math.cos(ang), radar_cy + radio * _math.sin(ang)

    for nivel_r in [1, 2, 3]:
        r_ring = radar_r * nivel_r / 3; pts_ring = [radar_punto(j, r_ring) for j in range(n_pts)]
        p = c.beginPath(); p.moveTo(*pts_ring[0])
        for px2, py2 in pts_ring[1:]: p.lineTo(px2, py2)
        p.close(); ring_colors = [(0.18,0.07,0.07), (0.18,0.14,0.04), (0.04,0.18,0.12)]; fill(ring_colors[nivel_r - 1]); c.drawPath(p, fill=1, stroke=0); stroke((0.12, 0.22, 0.38)); c.setLineWidth(0.4); c.drawPath(p, fill=0, stroke=1)

    stroke((0.15, 0.28, 0.48)); c.setLineWidth(0.6)
    for j in range(n_pts): ex, ey = radar_punto(j, radar_r); c.line(radar_cx, radar_cy, ex, ey)

    if puntos_prev:
        pts_prev_poly = [radar_punto(j, (min(puntos_prev.get(k, 0), 10) / 10) * radar_r) for j, k in enumerate(cats)]
        p = c.beginPath(); p.moveTo(*pts_prev_poly[0])
        for px2, py2 in pts_prev_poly[1:]: p.lineTo(px2, py2)
        p.close(); fill((0.25, 0.30, 0.40)); c.drawPath(p, fill=1, stroke=0); stroke((0.45, 0.55, 0.70)); c.setLineWidth(1.2); c.drawPath(p, fill=0, stroke=1)

    pts_act_poly = [radar_punto(j, (min(v, 10) / 10) * radar_r) for j, (k, v) in enumerate(puntos_act.items())]
    p = c.beginPath(); p.moveTo(*pts_act_poly[0])
    for px2, py2 in pts_act_poly[1:]: p.lineTo(px2, py2)
    p.close(); fill((0.0, 0.42, 0.70)); c.drawPath(p, fill=1, stroke=0); stroke(CIAN); c.setLineWidth(2); c.drawPath(p, fill=0, stroke=1)

    for j, (k, v) in enumerate(puntos_act.items()):
        px2, py2 = radar_punto(j, (min(v, 10) / 10) * radar_r); fill(CIAN); c.circle(px2, py2, 3.5, fill=1, stroke=0)
        lx, ly = radar_punto(j, radar_r + 14); fill(GRIS_TEXTO); c.setFont("Helvetica", 7); c.drawCentredString(lx, ly - 3, k)

    dx = 260; dy = y_seccion - 10
    fill(CIAN); c.setFont("Helvetica-Bold", 8); c.drawString(dx, dy, "DETALLE COMPARATIVO"); dy -= 16

    for cat, val in puntos_act.items():
        rect_f(dx, dy - 4, W - dx - 20, 20, AZUL_FILA_A); fill(GRIS_TEXTO); c.setFont("Helvetica", 8); c.drawString(dx + 5, dy + 2, cat)
        color_v = VERDE if val >= 6.5 else NARANJA if val >= 5 else ROJO; fill(color_v); c.setFont("Helvetica-Bold", 9); c.drawRightString(dx + 120, dy + 2, f"{val:.1f}")
        if puntos_prev and cat in puntos_prev:
            vp = puntos_prev[cat]; diff = val - vp
            if diff > 0.2: fill(VERDE); arrow = f"▲ +{diff:.1f}"
            elif diff < -0.2: fill(ROJO); arrow = f"▼ {diff:.1f}"
            else: fill(GRIS_LABEL); arrow = "= ="
            c.setFont("Helvetica-Bold", 8); c.drawString(dx + 130, dy + 2, arrow); fill(GRIS_LABEL); c.setFont("Helvetica", 7); c.drawRightString(W - 26, dy + 2, f"ant: {vp:.1f}")
        dy -= 22

    if puntos_prev: dy -= 4; fill(GRIS_LABEL); c.setFont("Helvetica", 7); c.drawString(dx, dy, "▲▼ variación respecto evaluación anterior")
    ...
    c.drawString(20, 10, "Bio Sport Pro  ·  Evaluación Deportiva de Alto Rendimiento"); c.drawRightString(W - 20, 10, f"Página 2 de 2  ·  {datos['fecha']}")
...
def generar_pdf_grupal(df: pd.DataFrame) -> BytesIO:
    ...
    c.setFillColorRGB(0, 0.83, 1); c.setFont("Helvetica-Bold", 18); c.drawString(40, alto - 50, "⚡ BIO SPORT PRO — INFORME GRUPAL")
    c.setFont("Helvetica", 10); c.setFillColorRGB(0.5, 0.7, 0.9)
    fecha_hoy = datetime.now().strftime("%d/%m/%Y"); c.drawString(40, alto - 70, f"Generado el {fecha_hoy}  ·  {len(df)} atletas evaluados")
    ...
    for _, row in df_show.iterrows(): tabla_data.append([str(row[c])[:18] for c in cols_mostrar])
    ...
    t.setStyle(TableStyle([
        ...
        ("BACKGROUND",   (0, 1), (-1, -1), colors.HexColor("#0d1627")), ("ROWBACKGROUNDS",(0, 1), (-1, -1), [colors.HexColor("#111827"), colors.HexColor("#0d1627")]),
        ("TEXTCOLOR",    (0, 1), (-1, -1), colors.HexColor("#c8d8f0")), ("FONTSIZE",     (0, 1), (-1, -1), 8),
        ("ALIGN",        (2, 0), (-1, -1), "CENTER"), ("GRID",         (0, 0), (-1, -1), 0.3, colors.HexColor("#1f2d45")),
        ("ROWHEIGHT",    (0, 0), (-1, -1), 16), ("TOPPADDING",   (0, 0), (-1, -1), 3), ("BOTTOMPADDING",(0, 0), (-1, -1), 3),
    ]))
    t.wrapOn(c, ancho - 80, alto); t.drawOn(c, 40, alto - 150 - len(tabla_data) * 17)

    c.setFillColorRGB(0.04, 0.07, 0.13); c.rect(0, 0, ancho, 30, fill=1, stroke=0); c.setFont("Helvetica", 7); c.setFillColorRGB(0.35, 0.50, 0.70)
    c.drawString(40, 10, "Generado por Bio Sport Pro  ·  Informe Grupal"); c.drawRightString(ancho - 40, 10, fecha_hoy)
...
with tab_eval:
    ...
    with c4: 
        deporte = st.selectbox(
            "Deporte / Disciplina", 
            options=list(BAREMOS_DEPORTIVOS.keys()), 
            index=0,
            key="in_dep"
        )
    ...
    if st.session_state.informe_actual:
        ...
        max_cmj = BAREMOS_DEPORTIVOS.get(d["deporte"], BAREMOS_DEPORTIVOS["General / Recreacional"])["CMJ"]
        max_rsi = BAREMOS_DEPORTIVOS.get(d["deporte"], BAREMOS_DEPORTIVOS["General / Recreacional"])["RSI"]
